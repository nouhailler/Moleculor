/* Moleculor — molecular composition database (values ~ per 100 g, public
   nutrition sources). Ported from the design handoff's foodData.js with the
   schema kept intact (see ./types.ts). Builder internals stay loosely typed,
   matching the original; the exported FOOD_DB is fully typed.

   ⚠️ Values are demonstration-grade orders of magnitude. For clinical use,
   swap this module for a validated source behind data/repository.ts while
...
// ── Molecular families: key → label + color (editorial oklch, constant chroma) ──
...
// molecule shorthand: [name, formula, famKey, valence, abundance, role, detail?]
// micro shorthand:    [name, val, unit, pct]
// system shorthand:   [name, valence, intensity(1..3), note]
function parseAb(str: unknown): Qty {
  if (str == null) return { qual: '' };
  const m = String(str).match(/(\d+(?:[.,]\d+)?)\s*(g|mg|µg|kcal)/);
  if (!m) return { qual: String(str) };
  return {
    n: parseFloat(m[1].replace(',', '.')),
    unit: m[2],
    approx: /≈|<|~/.test(String(str)),
  };
}

function mol(a: any[]): Molecule {
  return {
    name: a[0],
    formula: a[1],
    fam: a[2],
    family: FAMILIES[a[2]].label,
    valence: a[3],
    abundance: a[4],
    qty: parseAb(a[4]),
    role: a[5],
...
function genTimeline(f: Food): TimelinePhase[] {
  const ab = f.molecules
    .filter((m) => m.valence === 'b')
    .slice(0, 3)
    .map((m) => m.name);
  return [
...
interface CatInfo { f: string; v: Valence; r: string }
...
interface MicroInfo { f: string; v: Valence; r: string }
const MICRO_INFO: Record<string, MicroInfo> = {
...
interface FaInfo { f: string; cat: string; v: Valence; share: number; r: string }
const FA_DB: Record<string, FaInfo> = {
...
const AA_LIST = ['Leucine', 'Lysine', 'Valine', 'Phénylalanine', 'Thréonine', 'Tryptophane'];
interface AaInfo { f: string; share: number; r: string }
const AA_DB: Record<string, AaInfo> = {
...
function buildTree(f: Food): TreeNode[] {
  const byFam: Record<string, Molecule[]> = {};
  f.molecules.forEach((x) => {
    (byFam[x.fam] = byFam[x.fam] || []).push(x);
  });
  function qNum(n: number, unit?: string, approx?: boolean): Qty {
    return { n, unit: unit || 'g', approx: !!approx };
  }
  function catNode(id: string, label: string, fam: string, grams: number | null, children?: TreeNode[], dyn?: Dyn | null): TreeNode {
    const ci = CAT_INFO[id] || { f: '—', v: 'n' as Valence, r: '' };
    const qty: Qty = grams != null ? qNum(grams, 'g') : { qual: 'présent' };
    const info: Info = { name: label, formula: ci.f, fam, family: FAMILIES[fam].label, valence: ci.v, qty, detail: ci.r };
    if (dyn) info.dyn = dyn;
    return { id, label, fam, qty: grams != null ? qty : null, children: children || [], info };
  }
  function molLeaf(x: Molecule): TreeNode {
    return { id: 'm:' + x.name, label: x.name, fam: x.fam, qty: x.qty, mol: x.name, children: [], info: x };
  }
  function microLeaf(mi: Micro): TreeNode {
    const key = mi.name.replace(/\s*\(.*\)\s*/, '').trim();
    const mInfo = MICRO_INFO[key] || MICRO_INFO[mi.name] || { f: '—', v: 'b' as Valence, r: 'Micronutriment essentiel au bon fonctionnement de l’organisme.' };
    const qty = qNum(mi.val, mi.unit);
    return {
      id: 'mi:' + mi.name, label: mi.name, fam: 'micro', qty, children: [],
      info: { name: mi.name, formula: mInfo.f, fam: 'micro', family: 'Micronutriments', valence: mInfo.v, qty, pct: mi.pct, detail: mInfo.r, dyn: { type: 'micro', basePct: mi.pct } },
    };
  }
  function faLeaf(name: string, grams: number): TreeNode {
    const d = FA_DB[name];
    const ex = (byFam.lipides || []).filter((m) => m.name === name)[0];
    const qty: Qty = ex && ex.qty && ex.qty.n != null ? ex.qty : qNum(grams, 'g', true);
    return {
      id: 'fa:' + name, label: name, fam: 'lipides', qty, children: [],
      info: { name, formula: ex ? ex.formula : d.f, fam: 'lipides', family: 'Acides gras', valence: ex ? ex.valence : d.v, qty, detail: (ex ? ex.detail : d.r) },
    };
  }
  function aaLeaf(name: string, grams: number): TreeNode {
    const d = AA_DB[name];
    const qty = qNum(grams, 'g', true);
    return {
      id: 'aa:' + name, label: name, fam: 'proteines', qty, children: [],
      info: { name, formula: d.f, fam: 'proteines', family: 'Acides aminés', valence: 'b', qty, detail: d.r },
    };
  }

  const tree: TreeNode[] = [];
  if (f.macros.lip) {
    const lipChildren: TreeNode[] = [];
    ([
      ['sat', 'Acides gras saturés', f.sat],
      ['mono', 'Mono-insaturés (AGMI)', f.mono],
      ['poly', 'Poly-insaturés (AGPI)', f.poly],
    ] as [string, string, number][]).forEach((su) => {
      const fas: TreeNode[] = [];
      Object.keys(FA_DB).forEach((nm) => {
        if (FA_DB[nm].cat === su[0]) {
          const g = +(su[2] * FA_DB[nm].share).toFixed(1);
          if (g >= 0.1) fas.push(faLeaf(nm, g));
        }
      });
      lipChildren.push(catNode(su[0], su[1], 'lipides', su[2], fas, su[0] === 'sat' ? { type: 'sat', base: f.sat } : null));
    });
    (byFam.lipides || []).forEach((m) => { if (!FA_DB[m.name]) lipChildren.push(molLeaf(m)); });
    tree.push(catNode('lip', 'Lipides', 'lipides', f.macros.lip, lipChildren));
  }
  if (f.macros.gluc)
    tree.push(
      catNode('glu', 'Glucides', 'glucides', f.macros.gluc, [
  ...
  if (f.macros.prot) {
    const proChildren: TreeNode[] = [];
    (byFam.proteines || []).forEach((m) => { if (!/^Prot[ée]ines/.test(m.name)) proChildren.push(molLeaf(m)); });
    if (f.macros.prot >= 1) AA_LIST.forEach((nm) => proChildren.push(aaLeaf(nm, +(f.macros.prot * AA_DB[nm].share).toFixed(2))));
    tree.push(catNode('pro', 'Protéines', 'proteines', f.macros.prot, proChildren));
  }
  const micros = (f.micros || []).map(microLeaf);
  if (micros.length) tree.push(catNode('mic', 'Micronutriments', 'micro', null, micros));
  const bio = (byFam.bioactifs || []).map(molLeaf);
  if (bio.length) tree.push(catNode('bio', 'Composés bioactifs', 'bioactifs', null, bio));
  return tree;
}

export function makeFood(s: any): Food {
  const f: Food = {
    id: s.id, name: s.name, cat: s.cat, score: s.score, kcal: s.kcal, tagline: s.tagline,
    portion: s.portion || '100 g',
    macros: {
      lip: s.lip, gluc: s.gluc, fib: s.fib, prot: s.prot,
      autres: Math.max(0, +(100 - s.lip - s.gluc - s.fib - s.prot - (s.eau || 0)).toFixed(1)),
      eau: s.eau || 0,
    },
    sat: s.sat || 0, mono: s.mono || 0, poly: s.poly || 0, sucres: s.sucres || 0, amidon: s.amidon || 0,
    micros: (s.micros || []).map((a: any[]) => ({ name: a[0], val: a[1], unit: a[2], pct: a[3] })),
    molecules: (s.molecules || []).map((a: any[], i: number) => { const x = mol(a); x.id = s.id + '-' + i; return x; }),
    systems: (s.systems || []).map((a: any[]) => ({ name: a[0], valence: a[1], intensity: a[2], note: a[3] }) as SystemEntry),
    tree: [], timeline: [], benefits: [], risks: [],
  };
  f.tree = buildTree(f);
  f.timeline = s.timeline || genTimeline(f);
  f.benefits = f.molecules.filter((m) => m.valence === 'b').map((m) => ({ title: m.name, text: m.role, fam: m.fam }));
  f.risks = f.molecules.filter((m) => m.valence === 'r').map((m) => ({ title: m.name, text: m.role, fam: m.fam }));
  return f;
}

// compact helper: positional args → spec object
function cmp(
  id: string, name: string, cat: string, score: number, kcal: number,
  lip: number, gluc: number, fib: number, prot: number, eau: number,
  micros: any[], molecules: any[], systems: any[],
) {
  return {
    id, name, cat, score, kcal, lip, gluc, fib, prot, eau,
    ...
    micros, molecules, systems, tagline: '',
  };
}
...
export const FOOD_DB: FoodDB = { families: FAMILIES, foods, order };
